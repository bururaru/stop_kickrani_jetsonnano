...
def expand_box(x):
    width_x = int(x[2] - x[0])
    width_y = int(x[3] - x[1])
    x_ratio = 0.1  # 0.1 : 10% 크게
    y_ratio = 0.3  # 0.3 : 30% 크게
    x_var  = int(width_x * x_ratio)
    y_var = int(width_y * y_ratio)
    c1, c2 = (int(x[0]-x_var), int(x[1])-y_var), (int(x[2]+x_var), int(x[3])+y_var)
    return c1, c2
